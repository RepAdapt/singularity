...
#note RGID, SMID, LBID are generated using regex based on filename prefixes/suffixes, which vary depending on sequencing provider. These may vary depending on sequencing platform, so the variables above will work for NovaSeq sequenced reads from Genome Quebec.
